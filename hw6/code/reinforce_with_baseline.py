...
class ReinforceWithBaseline(tf.keras.Model):
    ...
    def value_function(self, states):
        ...
        critic1_out = self.critic1(states)
        values = self.critic2(critic1_out)
        return values
    ...
    def loss(self, states, actions, discounted_rewards):
        ...
        probs = tf.squeeze(self.call(states))
        ...
        idx = np.reshape(actions, [episode_length, 1])
        pa = tf.gather_nd(probs, idx, batch_dims=1)
